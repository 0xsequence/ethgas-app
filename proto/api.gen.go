...
func WebRPCSchemaHash() string {
	return "57299421d7d9aa1e9a85f991b5130fd875b0939a"
}
...
type ETHGas interface {
	...
	History(ctx context.Context) (map[uint64][]uint64, error)
}
...
func (s *eTHGasServer) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	case "/rpc/ETHGas/SuggestedGasPrice":
		...
		return
	case "/rpc/ETHGas/History":
		s.serveHistory(ctx, w, r)
		return
	default:
...
func (s *eTHGasServer) serveHistory(ctx context.Context, w http.ResponseWriter, r *http.Request) {
	header := r.Header.Get("Content-Type")
	i := strings.Index(header, ";")
	if i == -1 {
		i = len(header)
...
func (s *eTHGasServer) serveHistoryJSON(ctx context.Context, w http.ResponseWriter, r *http.Request) {
	var err error
	ctx = context.WithValue(ctx, MethodNameCtxKey, "History")
	...
	func() {
		...
		ret0, err = s.ETHGas.History(ctx)
	}()
	respContent := struct {
		Ret0 map[uint64][]uint64 `json:"data"`
	}{ret0}
...
func (c *eTHGasClient) History(ctx context.Context) (map[uint64][]uint64, error) {
	out := struct {
		Ret0 map[uint64][]uint64 `json:"data"`
	}{}
